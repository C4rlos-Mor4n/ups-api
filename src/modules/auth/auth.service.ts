...
export class AuthService {
  ...
  async requestCode(dto: RequestCodeDto): Promise<{ message: string; devCode?: string }> {
    ...
    await this.prisma.$transaction(async (tx) => {
      await tx.authVerificationCode.deleteMany({ where: { email: dto.email } });
      await tx.authVerificationCode.create({
        data: {
          email: dto.email,
          codeHash,
          expiresAt,
  ...
  async refresh(dto: RefreshTokenDto): Promise<AuthTokensDto> {
    ...
    const newTokens = await this.createSessionAndTokens(session.user);
